...
export default async function SeuDiaPage({ searchParams }: PageProps) {
  ...
  if (parsed.kind === 'valid') {
    try {
      result = await fetchOnThisDay(parsed.value.month, parsed.value.day, parsed.value.year)
    } catch (err) {
      console.error('fetchOnThisDay error:', err)
      fetchError = 'Não foi possível carregar os dados históricos. Tente novamente em instantes.'
    }
  }
        ...
        <div className="flex flex-col gap-16 md:gap-24">
          ...
          {fetchError && (
            <div
              ...
              className="border-2 border-border bg-electric-orange p-6 shadow-shadow"
            >
              <p className="font-heading text-lg font-black">{fetchError}</p>
            </div>
          )}
